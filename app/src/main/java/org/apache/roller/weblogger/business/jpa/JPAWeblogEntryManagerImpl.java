...
public class JPAWeblogEntryManagerImpl implements WeblogEntryManager {
    ...
    /**
 * Reads "Tags: #word1 #word2" appended by AutoTagGeneratorStep,
 * removes it from the body, and sets them as proper Roller tags.
 */
private void extractAndSetInlineTags(WeblogEntry entry) {
    if (entry.getText() == null) return;

    String text = entry.getText();
    int tagLineIndex = text.lastIndexOf("\n\nTags: #");

    if (tagLineIndex == -1) return; // no auto-tags found

    // Extract the tag line e.g. "Tags: #intelligence #healthcare #learning"
    String tagLine = text.substring(tagLineIndex).trim();

    // Remove tag line from body
    entry.setText(text.substring(0, tagLineIndex).trim());

    // Parse out the words after "Tags: "
    String tagsPart = tagLine.replace("Tags:", "").trim();
    String[] tagTokens = tagsPart.split("\\s+");

    try {
        // Collect existing tags
        String existing = entry.getTagsAsString();
        Set<String> allTags = new java.util.LinkedHashSet<>();
        if (existing != null && !existing.isBlank()) {
            allTags.addAll(java.util.Arrays.asList(existing.trim().split("\\s+")));
        }
        // Add new auto-generated tags (strip the # prefix)
        for (String token : tagTokens) {
            String tag = token.startsWith("#") ? token.substring(1) : token;
            if (!tag.isBlank()) allTags.add(tag);
        }
        entry.setTagsAsString(String.join(" ", allTags));
    } catch (Exception e) {
        LOG.debug("extractAndSetInlineTags: failed to set tags: " + e.getMessage());
    }
}
    ...
    public void saveWeblogEntry(WeblogEntry entry) throws WebloggerException {
                ...
                // Feature 2: Transforming Feeds Pipeline (Chain of Responsibility)

    if ((entry.getStatus() == WeblogEntry.PubStatus.PUBLISHED
    || entry.getStatus() == WeblogEntry.PubStatus.PENDING)
    && entry.getText() != null
    && entry.getText().trim().split("\\s+").length > 10) {
        EntryProcessingPipeline pipeline = EntryProcessingPipeline.buildDefaultPipeline();
        pipeline.process(entry);
        extractAndSetInlineTags(entry);
    }
