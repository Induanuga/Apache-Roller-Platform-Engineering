...
import org.apache.roller.weblogger.pojos.UserWeblogEntryStar;
...
public class JPAUserManagerImpl implements UserManager {
    ...
    public boolean isWeblogStarred(User user, Weblog weblog) throws WebloggerException {
        TypedQuery<UserWeblogStar> q = strategy.getNamedQuery("UserWeblogStar.getByUserAndWeblog", UserWeblogStar.class);
    ...
    public void starWeblog(User user, Weblog weblog) throws WebloggerException {
        if (!isWeblogStarred(user, weblog)) {
            UserWeblogStar star = new UserWeblogStar(user, weblog);
            strategy.store(star);
        }
    }
    ...
    public void unstarWeblog(User user, Weblog weblog) throws WebloggerException {
        ...
        q.setParameter(2, weblog);
        try {
            UserWeblogStar star = q.getSingleResult();
            strategy.remove(star);
        } catch (NoResultException ignored) {}
    }

    @Override
    public List<Weblog> getStarredWeblogs(User user) throws WebloggerException {
        TypedQuery<UserWeblogStar> q = strategy.getNamedQuery("UserWeblogStar.getByUser", UserWeblogStar.class);
        q.setParameter(1, user);
        List<UserWeblogStar> stars = q.getResultList();
        List<Weblog> weblogs = new ArrayList<>();
        for (UserWeblogStar star : stars) {
            weblogs.add(star.getWeblog());
        }
        return weblogs;
    }
    ...
    public long getWeblogStarCount(Weblog weblog) throws WebloggerException {
        TypedQuery<Long> q = strategy.getNamedQuery("UserWeblogStar.getCountByWeblog", Long.class);
    ...
    @Override
    public long getWeblogStarCountFresh(Weblog weblog) throws WebloggerException {
        try {
            // Step 1: Clear the JPA L2 cache for UserWeblogStar
            EntityManager em = strategy.getEntityManager(true);
            EntityManagerFactory emf = em.getEntityManagerFactory();
            Cache cache = emf.getCache();
            
            // Evict all UserWeblogStar instances from cache
            cache.evict(UserWeblogStar.class);
            
            // Step 2: Query database fresh (bypassing cache)
            TypedQuery<Long> q = strategy.getNamedQuery("UserWeblogStar.getCountByWeblog", Long.class);
            q.setParameter(1, weblog);
            List<Long> results = q.getResultList();
            ...
            log.error("Error getting fresh weblog star count", e);
            // Fallback to regular method if fresh read fails
            return getWeblogStarCount(weblog);
        }
    }

    @Override
    public boolean isWeblogEntryStarred(User user, WeblogEntry entry) throws WebloggerException {
        TypedQuery<UserWeblogEntryStar> q = strategy.getNamedQuery("UserWeblogEntryStar.getByUserAndWeblogEntry", UserWeblogEntryStar.class);
        q.setParameter(1, user);
        q.setParameter(2, entry);
        try {
            q.getSingleResult();
            return true;
        } catch (NoResultException e) {
    ...
    @Override
    public void starWeblogEntry(User user, WeblogEntry entry) throws WebloggerException {
        if (!isWeblogEntryStarred(user, entry)) {
            UserWeblogEntryStar star = new UserWeblogEntryStar(user, entry);
            strategy.store(star);
        }
    }

    @Override
    public void unstarWeblogEntry(User user, WeblogEntry entry) throws WebloggerException {
        TypedQuery<UserWeblogEntryStar> q = strategy.getNamedQuery("UserWeblogEntryStar.getByUserAndWeblogEntry", UserWeblogEntryStar.class);
        q.setParameter(1, user);
        q.setParameter(2, entry);
        try {
            UserWeblogEntryStar star = q.getSingleResult();
            strategy.remove(star);
        } catch (NoResultException ignored) {}
    }

    @Override
    public List<WeblogEntry> getStarredWeblogEntries(User user, int offset, int length) throws WebloggerException {
        TypedQuery<UserWeblogEntryStar> q = strategy.getNamedQuery("UserWeblogEntryStar.getByUser", UserWeblogEntryStar.class);
        q.setParameter(1, user);
        if (offset != 0) {
        ...
        List<UserWeblogEntryStar> stars = q.getResultList();
        List<WeblogEntry> entries = new ArrayList<>();
        for (UserWeblogEntryStar star : stars) {
            entries.add(star.getWeblogEntry());
        }
        return entries;
    }

    @Override
    public long getWeblogEntryStarCount(WeblogEntry entry) throws WebloggerException {
        TypedQuery<Long> q = strategy.getNamedQuery("UserWeblogEntryStar.getCountByWeblogEntry", Long.class);
    ...
    @Override
    public long getWeblogEntryStarCountFresh(WeblogEntry entry) throws WebloggerException {
        try {
            // Step 1: Clear the JPA L2 cache for UserWeblogEntryStar
            EntityManager em = strategy.getEntityManager(true);
            EntityManagerFactory emf = em.getEntityManagerFactory();
            Cache cache = emf.getCache();
            
            // Evict all UserWeblogEntryStar instances from cache
            cache.evict(UserWeblogEntryStar.class);
            ...
            log.error("Error getting fresh weblog entry star count", e);
            // Fallback to regular method if fresh read fails
            return getWeblogEntryStarCount(entry);
        }
    }

    @Override
    @SuppressWarnings("unchecked")
    public List<Object[]> getTopStarredWeblogEntries(int limit) throws WebloggerException {
        Query q = strategy.getNamedQuery("UserWeblogEntryStar.getTopByStarCount");
    ...
    @Override
    @SuppressWarnings("unchecked")
    public List<Object[]> getTopStarredWeblogs(int limit) throws WebloggerException {
        Query q = strategy.getNamedQuery("UserWeblogStar.getTopByStarCount");
        q.setMaxResults(limit);
        return (List<Object[]>) q.getResultList();
    }
...
//-------------------------------------------------------------- role CRUD
    ...
    /**
     * Reset user's password with proper encryption.
     * This method handles password encryption if configured.
     *
     * @param user User whose password should be reset.
     * @param newPassword The new password to be set (plain text).
     * @throws WebloggerException If there is a problem.
     */
    @Override
    public void resetUserPassword(User user, String newPassword) throws WebloggerException {
        if (user == null) {
            throw new WebloggerException("User cannot be null");
        }
        if (newPassword == null) {
            throw new WebloggerException("New password cannot be null");
        }
     ...
     * @param action The action to check (e.g., "login", "weblog", "admin").
     * @return true if user has the specified permission, false otherwise.
     */
    @Override
    ...
    /**
     * Check if user has multiple global permissions.
     * 
    ...
    public boolean hasGlobalPermissions(User user, List<String> actions) {
        ...
        try {
            ...
            return checkPermission(perm, user);
        } catch (WebloggerException ex) {
            log.error("Error checking global permissions for user: " + user.getUserName(), ex);
            return false;
        }
    }
}