...
public final class EntryEdit extends UIAction {
    ...
    /**
     * Check if authenticated user has global admin permission.
     * This is a convenience method for JSPs to avoid direct calls to User.hasGlobalPermission().
     * @return true if user has global admin permission, false otherwise.
     */
    public boolean isAuthenticatedUserAdmin() {
        try {
            return WebloggerFactory.getWeblogger().getUserManager()
                    .hasGlobalPermission(getAuthenticatedUser(), "admin");
        } catch (Exception ex) {
            log.error("Error checking admin permission", ex);
            return false;
        }
    }
