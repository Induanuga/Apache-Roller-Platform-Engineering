...
<%-- Bug Report / Dashboard buttons - always visible for logged in users --%>
<div class="row" style="margin-bottom:10px;">
    <div class="col-md-12 text-right">
        <a href="<s:url action="bugReport" namespace="/roller-ui/authoring"/>?weblog=<s:property value="existingPermissions.isEmpty ? '' : existingPermissions[0].weblog.handle"/>" class="btn btn-default">
            <span class="glyphicon glyphicon-warning-sign" aria-hidden="true"></span>
            Report an Issue
        </a>
        <s:if test="userIsAdmin">
            <a href="<s:url action="bugDashboard" namespace="/roller-ui/admin"/>" class="btn btn-danger">
        ...
        <%-- PROMPT: Welcome... you have no blog --%>
            <s:if test="existingPermissions.isEmpty && pendingPermissions.isEmpty">
                <p>
                    <s:text name="yourWebsites.prompt.noBlog" />
            ...
            <%-- PROMPT: You have invitation(s) --%>
                <s:elseif test="! pendingPermissions.isEmpty">
                    <p>
                        <s:text name="yourWebsites.invitationsPrompt" />
                    </p>
                ...
                <%-- PROMPT: default ... select a weblog to edit --%>
                    <s:else>
                        <p class="subtitle">
                            <s:text name="yourWebsites.prompt.hasBlog" />
                        </p>
                    </s:else>

                <%-- STARRED/TRENDING BUTTONS ROW --%>
                        <div class="row">
                            <div class="col-md-12 text-right">
                                <p>
                                    <a href="<s:url action='trendingBlogs'/>" class="btn btn-warning">
                                        <span class="glyphicon glyphicon-fire" aria-hidden="true"></span>&nbsp;Trending
                                    </a>&nbsp;
                                    <a href="<s:url action='starredWeblogEntries'/>" class="btn btn-primary">View
                                        Starred Blogposts</a>
                                </p>
                            </div>
                        </div>
                        ...
                        <s:if test="! starredWeblogs.isEmpty">
                            <div class="row">
                                <div class="col-md-12">
                                    <h2>
                                        <s:text name="Starred Weblogs" />
                                    </h2>
                                </div>
                            </div>

                            <s:iterator var="starredBlog" value="starredWeblogs">
                                <div class="well yourWeblogBox">
                                    <h3 class="mm_weblog_name">
                                        <span class="glyphicon glyphicon-star" aria-hidden="true"></span>
                                        &nbsp;
                                        <s:property value="#starredBlog.name" />
                                    </h3>
                                    <p> <a href='<s:property value="#starredBlog.absoluteURL" />'>
                                            <s:property value="#starredBlog.absoluteURL" />
                                        </a></p>

                                    <s:set var="latestEntry" value="getLatestEntry(#starredBlog)" />
                                    <s:if test="#latestEntry != null">
                                        <p>Last posted to on:
                                            <s:property value="#latestEntry.formatPubTime('yyyy-MM-dd HH:mm:ss')" />
                                        </p>
                                    </s:if>
                                    <p><a
                                            href="<s:url action='star' method='weblog'><s:param name='id' value='#starredBlog.id'/></s:url>">Remove
                                            Star</a></p>
                                </div>
                            </s:iterator>
                        </s:if>
                            ...
                            <s:if test="! existingPermissions.isEmpty">
                                ...
                                <s:iterator var="perms" value="existingPermissions">
                                    ...
                                    <div class="well yourWeblogBox">
                                        ...
                                        <p>
                                            <s:property value="#perms.weblog.about" escapeHtml="false" />
                                        </p>
                                        ...
                                        <p>You have
                                            ...
                                            <s:text name='yourWebsites.permission' />
                                        </p>

                                        <div class="btn-group" role="group" aria-label="...">

                                            <%-- New entry button --%>
                                                <s:url action="entryAdd" namespace="/roller-ui/authoring"
                                                    var="newEntry">
                                                ...
                                                <s:if test='!(#perms.hasAction("edit_draft"))'>
                                                    ...
                                                    <%-- Show Entries button with count for users above LIMITED
                                                        permission --%>
                                                        <s:url action="entries" namespace="/roller-ui/authoring"
                                                            var="editEntries">
                                                            <s:param name="weblog" value="#perms.weblog.handle" />
                                                        </s:url>
                                                        <s:a href="%{editEntries}" cssClass="btn btn-default">
                                                            ...
                                                            <span class="badge">
                                                                <s:property value="#perms.weblog.entryCount" />
                                                            </span>
                                                        </s:a>
                                                ...
                                                <s:if test='!(#perms.hasAction("edit_draft"))'>
                                                    ...
                                                    <%-- Show Comments button with count for users above LIMITED
                                                        permission --%>
                                                        <s:url action="comments" namespace="/roller-ui/authoring"
                                                            var="manageComments">
                                                            <s:param name="weblog" value="#perms.weblog.handle" />
                                                        </s:url>
                                                        <s:a href="%{manageComments}" cssClass="btn btn-default">
                                                            ...
                                                            <span class="badge">
                                                                <s:property value="#perms.weblog.commentCount" />
                                                            </span>
                                                        </s:a>
                                                ...
                                                <%-- Only admins get access to theme and config settings --%>
                                                        ...
                                                        <%-- And only show theme option if custom themes are enabled
                                                            ...
                                                            <%-- settings button --%>
                                                                <s:url action="weblogConfig"
                                                                    namespace="/roller-ui/authoring" var="manageWeblog">
                                                                    <s:param name="weblog"
                                                                        value="#perms.weblog.handle" />
                                                                </s:url>
                                                    ...
                                                    <%-- don't allow last admin to resign from blog --%>
                                                        <s:if
                                                            test='!(#perms.hasAction("admin") && #perms.weblog.adminUserCount == 1)'>

                                                            <button type="button" class="btn btn-default">
                                                                <span class="glyphicon glyphicon-trash"
                                                                    aria-hidden="true"></span>
                                                                <s:url action="memberResign"
                                                                    namespace="/roller-ui/authoring" var="resignWeblog">
                                                                    <s:param name="weblog"
                                                                        value="#perms.weblog.handle" />
                                                                </s:url>
